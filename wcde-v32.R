...
d <- unzip(zipfile = "C:\\Users\\Guy\\OneDrive - IIASA\\merged.zip", list = TRUE) %>%
  ...
  mutate(
    ...
    dest = paste0("./wcde-v32-single/",s,"/",i))
...
# check
x <- dir_info("wcde-v32-single", recurse = TRUE, type = "file") %>%
...
# copy v31 with no past data to v32
str_replace_all(no_past, pattern = "v31", replacement = "v32") %>%
