...
## wcde-v3: single .rds files from dropbox/onedrive zip
##
...
d <- unzip(zipfile = "C:\\Users\\Guy\\OneDrive - IIASA\\download.zip", list = TRUE) %>%
  as_tibble() %>%
  rename(file = 1) %>%
  ...
  mutate(
    ...
    i = str_sub(string = file, start = 11, end = -5),
    s = case_when(
    ...
    dest = paste0("./wcde-v33-single/",s,"/",i))
...
for(i in 1:nrow(d)){
  ...
  d0 <- unz(description = "C:\\Users\\Guy\\OneDrive - IIASA\\download.zip",
            #description = "C:\\Users\\Guy\\OneDrive - IIASA\\merged.zip",
...
# check
x <- dir_info("wcde-v33-single", recurse = TRUE, type = "file") %>%
...
# what is in v32, not in v33
v32 <- dir_ls("wcde-v32-single", recurse = TRUE, type = "file")
v33 <- dir_ls("wcde-v33-single", recurse = TRUE, type = "file")

setdiff(str_sub(string = v32, start = 16),
        str_sub(string = v33, start = 16))

setdiff(str_sub(string = v32, start = 17),
        str_sub(string = v33, start = 17)) %>%
  str_subset(pattern = "pop", negate = TRUE)

# # these are data without past values. bring from v31
# setdiff(str_sub(string = v31, start = 17),
#         str_sub(string = v32, start = 17)) %>%
#   str_subset(pattern = "pop", negate = TRUE) %>%
# #  str_sub(start = 3) %>%
#   str_extract("(?<=/)(.*)") %>%
#   str_extract("^[^/]+") %>%
...
#
# # create dir
# str_replace_all(no_past, pattern = "v32", replacement = "v33") %>%
#   str_extract("^.*/(?=[^/]*$)") %>%
#   unique() %>%
#   dir_create()
#
# # copy v32 with no past data to v33
# str_replace_all(no_past, pattern = "v32", replacement = "v32") %>%
#   file_copy(path = no_past, new_path = .)
